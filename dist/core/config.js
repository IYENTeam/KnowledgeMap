...
export class ConfigManager {
     ...
     */
    validateDirectory(path, name, autoFix) {
        const issues = [];
        const autoFixed = [];
        // 존재 여부 확인
        if (!existsSync(path)) {
...
export async function initializeEnvironment() {
    ...
    if (!validation.valid) {
        ...
        throw ConfigurationError.invalidPath(config.vaultPath, errors.map((e) => e.message).join('; '));
    }
