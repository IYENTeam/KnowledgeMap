...
export class KnowledgeOSError extends Error {
    ...
    recoveryHints;
    cause;
    constructor(message, context = {}, recoveryHints = [], cause) {
        super(message);
...
export class ConfigurationError extends KnowledgeOSError {
    code = ErrorCodes.CONFIG_INVALID_PATH;
    httpStatus = 500;
    ...
    static missingEnv(varName, defaultValue) {
        const error = new ConfigurationError(`Missing environment variable: ${varName}`, { varName, defaultValue }, [
            {
                action: 'set_env',
        ...
        error.code = ErrorCodes.CONFIG_MISSING_ENV;
        return error;
    }
    static permissionDenied(path) {
        const error = new ConfigurationError(`Permission denied: ${path}`, { path }, [
            {
        ...
        error.code = ErrorCodes.CONFIG_PERMISSION_DENIED;
        return error;
    }
}
export class LayoutError extends KnowledgeOSError {
    context;
    code = ErrorCodes.LAYOUT_ALLOCATION_FAILED;
    httpStatus = 422;
    constructor(message, context, recoveryHints = [], cause) {
        super(message, context, recoveryHints, cause);
        this.context = context;
    }
    static allocationFailed(context) {
        return new LayoutError(`Failed to allocate node position in zone ${context.zone}`, context, [
            {
    ...
    static anchorNotFound(anchorId, canvasPath) {
        const error = new LayoutError(`Anchor node not found: ${anchorId}`, { zone: 'unknown', anchorId, existingNodeCount: 0, canvasPath }, [
            {
        ...
        error.code = ErrorCodes.LAYOUT_ANCHOR_NOT_FOUND;
        return error;
    }
    static zoneSaturated(zone, nodeCount) {
        const error = new LayoutError(`Zone ${zone} is saturated with ${nodeCount} nodes`, { zone, anchorId: '', existingNodeCount: nodeCount }, [
            {
        ...
        error.code = ErrorCodes.LAYOUT_ZONE_SATURATED;
        return error;
    }
}
// =============================================================================
...
export class CanvasError extends KnowledgeOSError {
    code = ErrorCodes.CANVAS_NOT_FOUND;
    httpStatus = 404;
    static notFound(canvasPath) {
        return new CanvasError(`Canvas not found: ${canvasPath}`, { canvasPath }, [
            {
                action: 'create_new',
    ...
    static parseError(canvasPath, cause) {
        const error = new CanvasError(`Failed to parse canvas: ${canvasPath}`, { canvasPath, parseError: cause.message }, [
            {
        ...
        error.code = ErrorCodes.CANVAS_PARSE_ERROR;
        return error;
    }
    static saveError(canvasPath, cause) {
        const error = new CanvasError(`Failed to save canvas: ${canvasPath}`, { canvasPath, saveError: cause.message }, [
            {
        ...
        error.code = ErrorCodes.CANVAS_SAVE_ERROR;
        return error;
    }
}
// =============================================================================
...
export class ToolError extends KnowledgeOSError {
    ...
    httpStatus = 500;
    static validationError(toolName, errors) {
        const error = new ToolError(`Validation failed for tool: ${toolName}`, { toolName, validationErrors: errors }, [
            {
        ...
        error.code = ErrorCodes.TOOL_VALIDATION_ERROR;
        return error;
    }
    static executionError(toolName, cause) {
        return new ToolError(`Execution failed for tool: ${toolName}`, { toolName, originalError: cause.message }, [
            {
                action: 'retry',
    ...
    static notFound(toolName) {
        const error = new ToolError(`Tool not found: ${toolName}`, { toolName }, [
            {
                action: 'list_tools',
                description: 'List available tools to find the correct name',
            },
        ]);
        error.code = ErrorCodes.TOOL_NOT_FOUND;
        return error;
    }
}
// =============================================================================
// Error Utilities
// =============================================================================
export function isKnowledgeOSError(error) {
