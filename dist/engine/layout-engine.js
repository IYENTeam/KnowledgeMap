 ...
 * - 구조화된 에러 시스템
 */
import { ZONE_SPECS, getNodeSize, calculateTextNodeSize, shouldCreateEdge, } from '../types/index.js';
...
const nodeMetadataStore = new WeakMap();
// =============================================================================
// Layout Engine Class
// =============================================================================
export class LayoutEngine {
     ...
     */
    allocateByRelation(options) {
        const { anchorId, relation, content, color } = options;
        // 1. Anchor 노드 찾기
        const anchor = this.findNode(anchorId);
        if (!anchor) {
            throw LayoutError.anchorNotFound(anchorId);
        }
        ...
        const allocation = this.allocateInZoneWithFallback(anchor, primaryZone, nodeSize);
        if (!allocation) {
            // 모든 시도 실패 시 상세한 에러
        ...
        if (shouldCreateEdge(relation)) {
            ...
            edge = CanvasParser.createEdge(anchorId, nodeId, {
     ...
     */
    allocateMultiple(anchorId, items) {
        const results = [];
    ...
    // ===========================================================================
    allocateInZoneWithFallback(anchor, primaryZone, nodeSize) {
        // 1. Primary Zone에서 시도
        ...
        for (const zone of SemanticRouter.getAllZones()) {
            if (zone === primaryZone || adjacentZones.includes(zone))
                continue;
    ...
    findPositionInZone(anchor, zone, nodeSize) {
        ...
        const simplePosition = this.trySimplePosition(baseX, baseY, zoneNodes, nodeSize, anchor);
        if (simplePosition && !this.checkCollision(simplePosition)) {
            return simplePosition;
        }
        // 2. 나선형 탐색
        const spiralPosition = this.spiralSearch(baseX, baseY, nodeSize, zone, spec.dx, spec.dy);
        if (spiralPosition) {
            return spiralPosition;
        }
        return null;
    }
    trySimplePosition(baseX, baseY, zoneNodes, nodeSize, _anchor) {
        ...
        const avgHeight = zoneNodes.reduce((sum, n) => sum + n.height, 0) / zoneNodes.length;
        return {
     ...
     */
    spiralSearch(centerX, centerY, nodeSize, zone, dirX, dirY) {
    ...
    *generateSpiralPoints(ring) {
    ...
    getSpiralDirections(dirX, dirY) {
        // Zone 방향에 따라 탐색 우선순위 조정
        if (dirY > 0)
            return [[0, 1], [1, 0], [-1, 0], [0, -1]]; // SOUTH
        if (dirY < 0)
            return [[0, -1], [1, 0], [-1, 0], [0, 1]]; // NORTH
        if (dirX > 0)
            return [[1, 0], [0, 1], [0, -1], [-1, 0]]; // EAST
        if (dirX < 0)
            return [[-1, 0], [0, 1], [0, -1], [1, 0]]; // WEST
        return [[0, 1], [1, 0], [0, -1], [-1, 0]]; // default
    }
    getAttemptedPositions(anchor, zone, nodeSize) {
        // 디버깅용: 시도한 위치들 반환
        const spec = ZONE_SPECS[zone];
        const { gridGap } = this.config;
        const baseX = anchor.x + spec.dx * (anchor.width + gridGap * 2);
    ...
    // Collision Detection
    // ===========================================================================
    checkCollision(candidate, excludeIds = []) {
    ...
    // Node Creation
    // ===========================================================================
    createNode(nodeId, content, position, color) {
        switch (content.type) {
            case 'text':
    ...
    expandGroup(zone) {
        ...
        group.x = bounds.minX - groupPadding;
        group.y = bounds.minY - groupPadding;
        group.width = bounds.width + 2 * groupPadding;
        group.height = bounds.height + 2 * groupPadding;
        return group;
    }
    // ===========================================================================
    // Zone Queries
     ...
     */
    createInitialLayout(topic, options) {
        // Topic 노드 (중앙)
