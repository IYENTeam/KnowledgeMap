...
export const ExpandCanvasSchema = z.object({
  ...
  useTopicAsAnchor: z.boolean().optional().describe(
    'true로 설정하면 anchorId를 무시하고 Topic(CORE) 노드를 기준으로 배치합니다. ' +
...
export const AddNodeSchema = z.object({
  ...
  useTopicAsAnchor: z.boolean().optional().describe(
    'true로 설정하면 anchorId를 무시하고 Topic(CORE) 노드를 기준으로 배치합니다. (기본: false)'
  ),
});
...
export class CanvasTools {
  ...
  constructor(canvasDir: string = '03_Canvas', vaultPath: string = '.') {
   ...
   * 새 캔버스 생성
   */
  async createCanvas(params: z.infer<typeof CreateCanvasSchema>): Promise<{
    canvasPath: string;
    topicNodeId: string;
    nodeCount: number;
    edgeCount: number;
    ...
    const topicNode = nodes.find(
      (n) => n.type === 'text' && (n as any).text?.startsWith('# ')
    );
   ...
   * 캔버스 확장 (여러 노드 추가)
   */
  async expandCanvas(params: z.infer<typeof ExpandCanvasSchema>): Promise<{
    addedNodes: string[];
    addedEdges: number;
    ...
    for (const item of items) {
      const content: { type: 'text' | 'file' | 'link'; text?: string; file?: string; url?: string } = {
        type: item.type,
      };
   ...
   * 단일 노드 추가
   */
  async addNode(params: z.infer<typeof AddNodeSchema>): Promise<{
    nodeId: string;
    edgeId: string | null;  // v2.1: 엣지 정책에 따라 null일 수 있음
    zone: string;
    usedTopicAnchor?: boolean;
  }> {
    ...
    const contentObj: { type: 'text' | 'file' | 'link'; text?: string; file?: string; url?: string } = {
      type,
    };
   ...
   * 캔버스 정보 조회
   */
  async getCanvasInfo(params: z.infer<typeof GetCanvasInfoSchema>): Promise<{
    topic: string;
    nodeCount: number;
    edgeCount: number;
    ...
    const nodeSummaries = canvas.nodes.slice(0, 20).map((node) => {
      let preview = '';
      if (node.type === 'text') {
        preview = ((node as any).text || '').slice(0, 50);
      } else if (node.type === 'file') {
        preview = (node as any).file || '';
      } else if (node.type === 'link') {
        preview = (node as any).url || '';
      } else if (node.type === 'group') {
        preview = `[Group: ${(node as any).label || 'unnamed'}]`;
      }
   ...
   * 질문 노드 목록 조회
   */
  async listQuestions(params: z.infer<typeof ListQuestionsSchema>): Promise<{
    questions: Array<{
      id: string;
      text: string;
      status: string;
    ...
    for (const node of canvas.nodes) {
      ...
      const isQuestion =
        text.includes('?') || color === '4' || nodeMeta?.role === 'question';

      if (!isQuestion) continue;

      const nodeStatus = nodeMeta?.status || 'pending';

      if (status !== 'all' && status !== nodeStatus) continue;
   ...
   * 질문 해결됨 표시
   */
  async resolveQuestion(params: z.infer<typeof ResolveQuestionSchema>): Promise<{
    success: boolean;
    questionId: string;
  }> {
   ...
   * 캔버스 결정화 (노트로 변환)
   */
  async crystallizeCanvas(params: z.infer<typeof CrystallizeCanvasSchema>): Promise<{
    outputPath: string;
    sections: number;
    wordCount: number;
  }> {
    ...
    const textNodes = canvas.nodes.filter((n) => n.type === 'text') as any[];
    ...
    if (format === 'outline') {
      ...
      for (const node of textNodes) {
        ...
        if (text.startsWith('# ')) continue; // Topic 제외
      ...
      // 참조 자료
      const fileNodes = canvas.nodes.filter((n) => n.type === 'file') as any[];
      ...
      const linkNodes = canvas.nodes.filter((n) => n.type === 'link') as any[];
