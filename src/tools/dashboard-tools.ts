...
export class DashboardTools {
   ...
   * 전체 대시보드 정보 조회
   */
  async getDashboard(params: z.infer<typeof GetDashboardSchema>): Promise<{
    overview: {
   ...
   * 특정 캔버스의 워크플로우 진행 상황
   */
  async getWorkflowProgress(params: z.infer<typeof GetWorkflowProgressSchema>): Promise<{
    canvasPath: string;
    ...
    switch (meta.workflow.state) {
      ...
      case 'expanded':
        if (meta.statistics.questions > meta.statistics.resolvedQuestions) {
          nextSteps.push(
            `${meta.statistics.questions - meta.statistics.resolvedQuestions}개의 미해결 질문이 있습니다`
          );
        }
   ...
   * 캔버스 목록 조회
   */
  async listCanvases(params: z.infer<typeof ListCanvasesSchema>): Promise<{
    canvases: Array<{
      path: string;
      topic: string;
      state: string;
   ...
   * 활동 로그 조회
   */
  async getActivityLog(params: z.infer<typeof GetActivityLogSchema>): Promise<{
    events: Array<{
      canvas: string;
      action: string;
      agent: string;
   ...
   * 대기 중인 작업 목록
   */
  async getPendingTasks(_params: z.infer<typeof GetPendingTasksSchema>): Promise<{
    tasks: Array<{
      canvas: string;
   ...
   * 시스템 상태 확인
   */
  async getSystemHealth(_params: z.infer<typeof GetSystemHealthSchema>): Promise<{
  ...
  private async findCanvases(): Promise<string[]> {
