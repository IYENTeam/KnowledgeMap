...
export class VaultTools {
  ...
  private readonly crossRefManager: CrossReferenceManager;

  constructor(vaultPath: string = '.', canvasDir: string = '03_Canvas') {
   ...
   * 노트 검색
   */
  async searchNotes(params: z.infer<typeof SearchNotesSchema>): Promise<{
    results: Array<{
      path: string;
      title: string;
      score: number;
   ...
   * 여러 키워드로 관련 노트 찾기
   */
  async findRelatedNotes(params: z.infer<typeof FindRelatedNotesSchema>): Promise<{
    results: Array<{
      path: string;
      title: string;
      score: number;
   ...
   * 노트 메타데이터 조회
   */
  async getNoteMetadata(params: z.infer<typeof GetNoteMetadataSchema>): Promise<{
    path: string;
    title: string;
    tags: string[];
    links: string[];
   ...
   * 볼트 인덱스 빌드
   */
  async buildIndex(params: z.infer<typeof BuildIndexSchema>): Promise<{
    totalNotes: number;
    totalTags: number;
   ...
   * 볼트 통계 조회
   */
  async getVaultStats(_params: z.infer<typeof GetVaultStatsSchema>): Promise<{
    totalNotes: number;
    totalTags: number;
    totalKeywords: number;
    indexedAt: string;
   ...
   * 관련 캔버스 찾기
   */
  async findRelatedCanvases(params: z.infer<typeof FindRelatedCanvasesSchema>): Promise<{
    canvases: Array<{
      path: string;
      score: number;
    }>;
   ...
   * 키워드로 캔버스 검색
   */
  async searchCanvases(params: z.infer<typeof SearchCanvasesSchema>): Promise<{
    canvases: string[];
  }> {
   ...
   * 캔버스 네트워크 데이터 조회
   */
  async getCanvasNetwork(_params: z.infer<typeof GetCanvasNetworkSchema>): Promise<{
    nodes: Array<{
      id: string;
      topic: string;
      size: number;
   ...
   * 캔버스 링크 제안
   */
  async suggestLinks(params: z.infer<typeof SuggestLinksSchema>): Promise<{
    suggestions: Array<{
      canvas: string;
      topic: string;
      reason: string;
   ...
   * 크로스 레퍼런스 통계
   */
  async getCrossRefStats(_params: z.infer<typeof GetCrossRefStatsSchema>): Promise<{
   ...
   * 크로스 레퍼런스 인덱스 빌드
   */
  async buildCrossRefIndex(force = false): Promise<{
    totalCanvases: number;
    indexedAt: string;
  }> {
