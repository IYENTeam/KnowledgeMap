...
export class CanvasParser {
   ...
   * .canvas 파일을 로드합니다.
   */
  static async load(filePath: string): Promise<CanvasDocument> {
  ...
  static async save(
    ...
    const cleanNodes = nodes.map((node) => {
      ...
      return clean as unknown as CanvasNode;
    });
   ...
   * 타입으로 노드들을 찾습니다.
   */
  static findNodesByType<T extends CanvasNode>(
    nodes: CanvasNode[],
    nodeType: T['type']
  ): T[] {
    return nodes.filter((n) => n.type === nodeType) as T[];
  }

  /**
   * 텍스트를 포함하는 노드들을 찾습니다.
   */
  static findNodesContainingText(
    nodes: CanvasNode[],
    searchText: string,
    caseSensitive = false
  ): TextNode[] {
    const textNodes = this.findNodesByType<TextNode>(nodes, 'text');
  ...
  static findQuestionNodes(nodes: CanvasNode[]): TextNode[] {
    return this.findNodesByType<TextNode>(nodes, 'text').filter((node) =>
      node.text.includes('?')
    );
  }
