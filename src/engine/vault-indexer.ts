...
export class VaultIndexer {
   ...
   * 전체 볼트 인덱스 빌드
   */
  async buildIndex(force = false): Promise<VaultIndex> {
    // 기존 인덱스 로드
    if (!force) {
      try {
   ...
   * 단일 노트 인덱싱
   */
  private async indexNote(filePath: string): Promise<NoteMetadata | null> {
    try {
      const relativePath = relative(this.vaultPath, filePath);
  ...
  private extractCreatedDate(content: string): string | undefined {
  ...
  async searchByKeyword(
    ...
    if (results.length < limit) {
      for (const [path, note] of Object.entries(this.index!.notes)) {
        if (results.find((r) => r.note.path === path)) continue;
  ...
  async findRelatedNotes(
    ...
    for (const keyword of keywords) {
      ...
      for (const path of keywordPaths) {
        if (!scores[path]) scores[path] = { score: 0, keywords: [] };
        scores[path].score += 2;
