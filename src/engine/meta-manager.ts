...
export class MetaManager {
  ...
  private getMetaPath(canvasPath: string): string {
   ...
   * 메타 파일 존재 여부 확인
   */
  async exists(canvasPath: string): Promise<boolean> {
    try {
   ...
   * 메타데이터 로드
   */
  async load(canvasPath: string, autoCreate = true): Promise<CanvasMeta> {
    const metaPath = this.getMetaPath(canvasPath);

    try {
      const content = await readFile(metaPath, 'utf-8');
      const meta = JSON.parse(content) as CanvasMeta;
   ...
   * 메타데이터 저장
   */
  async save(canvasPath: string, meta: CanvasMeta): Promise<void> {
   ...
   * 새 메타데이터 생성
   */
  async create(canvasPath: string): Promise<CanvasMeta> {
    await this.ensureMetaDir();
  ...
  private async indexCanvas(
    ...
    for (const node of nodes) {
      ...
      let status: 'active' | 'pending' | 'resolved' | 'archived' = 'active';
   ...
   * 노드의 역할 추론
   */
  private inferNodeRole(node: CanvasNode): NodeRole {
    const color = node.color;

    // 색상 기반 추론
    const colorRoles: Record<string, NodeRole> = {
      '1': 'command', // Red
      '2': 'context', // Orange
      '3': 'answer',  // Yellow
      '4': 'question', // Green
    ...
    // 텍스트 기반 추론
    if (node.type === 'text') {
      if (node.text.startsWith('# ')) return 'topic';
      if (node.text.includes('?')) return 'question';
    }

    if (node.type === 'file') return 'vaultNote';
    if (node.type === 'link') return 'resource';
   ...
   * 미해결 질문 노드 ID 목록
   */
  async getPendingQuestions(canvasPath: string): Promise<string[]> {
    const meta = await this.load(canvasPath);

    return Object.entries(meta.semanticGraph)
      .filter(([_, nodeMeta]) =>
        nodeMeta.role === 'question' && nodeMeta.status === 'pending'
      )
      .map(([nodeId]) => nodeId);
  }
  ...
  async getIntentNodes(
    ...
    return Object.entries(meta.semanticGraph)
      .filter(([_, nodeMeta]) =>
        nodeMeta.intent && nodeMeta.status === 'pending'
      )
      .map(([nodeId, nodeMeta]) => ({
        id: nodeId,
   ...
   * 워크플로우 상태 조회
   */
  async getWorkflowState(canvasPath: string): Promise<WorkflowState> {
    const meta = await this.load(canvasPath);
   ...
   * 통계 조회
   */
  async getStatistics(canvasPath: string): Promise<CanvasMeta['statistics']> {
    const meta = await this.load(canvasPath);
